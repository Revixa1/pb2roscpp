...
class ArduinoCommunicationNode : public rclcpp::Node
{
	public:
		...
		: Node("pb2ros") //is a Node
		{
			
			
			my_serialp=new serial::Serial(port,baud, serial::Timeout::simpleTimeout(1000));//start serial com with arduino
			std::this_thread::sleep_for(std::chrono::seconds(2)); //wait for 2 seconds to let it connect
			...
			publisher_ = this->create_publisher<std_msgs::msg::Float32MultiArray>("prop_sensors", 10);//create publisher object
			subscription_ = this->create_subscription<geometry_msgs::msg::Twist>("prop_cmd", 1, std::bind(&ArduinoCommunicationNode::controlCallback, this, _1));//create subscriber object

			timer_ = this->create_wall_timer(
			      std::chrono::milliseconds(15), std::bind(&ArduinoCommunicationNode::readMessage, this));//create the reading/publishing loop for the topic /prop_sensors
	...
	private:
		...
		serial::Serial* my_serialp; //Serial pointer to initialize the serial communication in the constructor
		...
		void controlCallback(const geometry_msgs::msg::Twist msg)
		{
		    ...
		    char toSend[10];
   			...
   			//stringify
            for(j= 0; j < (int)total_bytes_encoded; j++)
            {
	...
	void readMessage()//reading arduino messages callback
		{
				...
				if(my_serialp->available()>1000){//wait for 1000 characters to enter the serialbuffer
				

					my_serialp->readline(msgbuffer,MAX_MSG_LEN,">"); //readuntil the end of a message
				
					parseMsg((char*)msgbuffer.c_str(),msgids,msgs); //parse the message between Id and message parts
				
					

					charsToBytes(msgs[0],bufferin); //convert to bytes for decoding

					pb_istream_t istream = pb_istream_from_buffer(bufferin, (size_t)78); //seting up decoding stream,   78 is the size of the incomming message
					...
					{
						prop_sensors.data.push_back(decodedData.numbers[i]);//append the decoded message to the data
					}
		...
		static uint8_t charToHex(char in1, char in2) //Code from PBUtils.cpp (original pb2ros2 codebase) converts chars to hex for decoding used by charstobytes
		{
		  uint8_t val[2];
		  char in[2] = { in1, in2};
		...
		static void charsToBytes(char* inString, uint8_t* stringValue) //Code from PBUtils.cpp (original pb2ros2 codebase) converts chars read from serial port to bytes for decoding
		{
		...
		static void FloatList_add_number(FloatList * list, float number) //custom function to handle adding floats to float arrays in nanopb
		{
		...
		static bool FloatArray_decode_single_number(pb_istream_t *istream, const pb_field_t *field, void **arg)//custom decoding function from stackoverflow
		{
		...
		static bool FloatArray_encode_numbers(pb_ostream_t *ostream, const pb_field_t *field, void * const *arg)//custom encoding function from stackoverflow
		{
